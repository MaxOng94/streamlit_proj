...
# use cache_data, as long as the parameters within run_request are the same, the function will not be called again
@st.cache_data
def run_request(question_to_ask, model_type, go_id, key, alt_key):
    if model_type == "gpt-4" or model_type == "gpt-3.5-turbo" :
        ...
        # Hugging Face model
        llm = HuggingFaceHub(huggingfacehub_api_token = alt_key, repo_id="codellama/" + model_type, model_kwargs={"temperature":0.005, "max_new_tokens":500})
...
@st.cache_data
def get_primer_for_plotly(df_dataset,df_name):
    # Primer function to take a dataframe and its name
    # and the name of the columns
    # and any columns with less than 20 unique values it adds the values to the primer
    # and horizontal grid lines and labeling
    ...
    primer_desc = primer_desc + "\nLabel the x and y axes appropriately."
    primer_desc = primer_desc + "\nAdd a title. Set the fig suptitle as empty."
    primer_desc = primer_desc + "\nDo any fig update at the end of the code."
    primer_desc = primer_desc + "\nDo not end your code with fig.show()."
    primer_desc = primer_desc + "\nInstead, end your code with ``st.plotly_chart(fig, theme='streamlit', use_container_width=True).``"
    primer_desc = primer_desc + "\nCreate a visualization in streamlit only using plotly and the dataframe df to graph the following: "
